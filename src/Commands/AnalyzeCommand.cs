    ...
    private static void PrintAnalysisResults(AnalysisResult result)
    {
        ...
        if (result.TotalPotentialDeadClasses > 0 || result.TotalPotentialDeadMethods > 0)
        {
            ...
            if (result.TotalPotentialDeadClasses > 0)
            {
                ...
                Console.WriteLine($"Potentially Dead Classes: {result.TotalPotentialDeadClasses} ({result.DeadClassPercentage:F1}% of all classes)");
                Console.ResetColor();
            }
            ...
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine($"Potentially Dead Methods: {result.TotalPotentialDeadMethods} ({result.DeadMethodPercentage:F1}% of all methods)");
                Console.ResetColor();
            }
